		...
		* addIndexesNoOptimize(). */
		...
		private void  _MergeInit(MergePolicy.OneMerge merge)
		{
			lock (this)
			{
				...
				string currentDocStoreSegment = docWriter.DocStoreSegment;
				...
				for (int i = 0; i < end; i++)
				{
					...
					string docStoreSegment = si.DocStoreSegment;
					if (docStoreSegment == null)
						mergeDocStores = true;
		...
		public sealed class MaxFieldLength
		{
			...
			/// </param>
			internal MaxFieldLength(string name, int limit)
			{
				this.name = name;
				this.limit = limit;
			}
		...
		public /*internal*/ virtual bool TestPoint(string name)
		{
			return true;
		}
