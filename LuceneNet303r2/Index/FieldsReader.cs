...
namespace Lucene.Net.Index
{
	...
	public sealed class FieldsReader : ICloneable, IDisposable
	{
		...
		internal FieldsReader(Directory d, string segment, FieldInfos fn, int readBufferSize):this(d, segment, fn, readBufferSize, - 1, 0)
		{
		}
		...
		private sealed class LazyField : AbstractField
		{
			...
			public LazyField(FieldsReader enclosingInstance, string name, Field.Store store, int toRead, long pointer, bool isBinary, bool isCompressed):base(name, store, Field.Index.NO, Field.TermVector.NO)
			{
			...
			public LazyField(FieldsReader enclosingInstance, string name, Field.Store store, Field.Index index, Field.TermVector termVector, int toRead, long pointer, bool isBinary, bool isCompressed):base(name, store, index, termVector)
			{
				InitBlock(enclosingInstance);
				this.toRead = toRead;
				this.pointer = pointer;
